...
class LoginViewModel(private val loginUseCase: LoginUseCase, private val mContext: Context): ViewModel() {
    ...
    val sharepref = this.mContext.getSharedPreferences("USER_BUSCHECK", Context.MODE_PRIVATE)
    ...
    fun onSavedID(id: String,mobileUserID: Int,name: String){
        sharepref.edit().putString("USER_ID", id).commit()
        sharepref.edit().putInt("MobileUserID", mobileUserID).commit()
        sharepref.edit().putString("Name", name).commit()
